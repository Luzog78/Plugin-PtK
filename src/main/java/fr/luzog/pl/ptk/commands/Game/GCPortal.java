...
import fr.luzog.pl.ptk.guis.GuiPortals;
...
public class GCPortal {
    public static final String syntaxe = "/" + Main.CMD + " portal [help | (nether | end) [info | open | close]]"
            + "\n§cou /" + Main.CMD + " portal (nether | end) (cooldown <cooldown> | openingMat <material> | closingMat <material> | data <data>)"
            + "\n§cou /" + Main.CMD + " portal (nether | end) (over | dim) (spawn | pos1 | pos2) (get | reset | set <x> <y> <z> [<yw> <pi>] [<world>])";
    ...
    public static boolean onCommand(CommandSender sender, Command cmd, String msg, String[] args) {
        ...
        if (args.length == 0)
            ...
            if (sender instanceof Player)
                u.getPlayer().openInventory(GuiPortals.getMainInventory(Main.CMD));
            else
            ...
            u.synt();
                ...
                && (p = type == GuiPortals.PortalType.NETHER ? GManager.getCurrentGame().getNether()
                : type == GuiPortals.PortalType.END ? GManager.getCurrentGame().getEnd() : null) != null) {

            if (args.length == 2)
                if (sender instanceof Player)
                    u.getPlayer().openInventory(GuiPortals.getPortalInventory(
                            p, type, u.getPlayer().getLocation(), Main.CMD + " portal"));
                else
                    u.err(CmdUtils.err_not_player);
                ...
                if (type == GuiPortals.PortalType.NETHER)
                    GManager.getCurrentGame().saveNether();
                else if (type == GuiPortals.PortalType.END)
                    GManager.getCurrentGame().saveEnd();
                ...
                if (args.length == 3)
                    u.err("/" + Main.CMD + " portal " + p.getName() + " cooldown <cooldown>");
                ...
                if (args.length == 3)
                    u.err("/" + Main.CMD + " portal " + p.getName() + " openingMat <material>");
                else if (args.length == 4) {
                    try {
                ...
                if (args.length == 3)
                    u.err("/" + Main.CMD + " portal " + p.getName() + " closingMat <material>");
                else if (args.length == 4) {
                    try {
                        Material mat = Material.valueOf(args[3]);
                        boolean opened = p.isOpened();
                ...
                if (args.length == 3)
                    u.err("/" + Main.CMD + " portal " + p.getName() + " data <data>");
                else if (args.length == 4) {
                    try {
